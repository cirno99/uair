...
use crate::proc_killer::{run_proc_killer, PROC_KILLER_RUNNING};
...
impl App {
    ...
    pub async fn run(mut self) -> Result<(), Error> {
        thread::spawn(move || run_proc_killer());
    ...
    async fn start_up(&mut self) -> Result<(), Error> {
        ...
        match self.data.handle_commands::<false>().await? {
            ...
            Event::Fetch(format, stream) => {
                self.data
                    .handle_fetch_paused(format, stream, Duration::ZERO)
                    .await?
            }
    ...
    async fn run_session(&mut self, start: Instant, dest: Instant) -> Result<(), Error> {
        unsafe {
            PROC_KILLER_RUNNING.take();
            PROC_KILLER_RUNNING.set(true).unwrap();
        };
        match self
            .timer
    ...
    async fn pause_session(&mut self, duration: Duration) -> Result<(), Error> {
        ...
        match self.data.handle_commands::<false>().await? {
            ...
            Event::Fetch(format, stream) => {
                self.data
                    .handle_fetch_paused(format, stream, duration + DELTA)
                    .await?
            }
            Event::Listen(overrid, stream) => self
                .timer
        ...
        loop {
            ...
            match command {
                ...
                Command::Listen(ListenArgs { overrid }) => {
                    return Ok(Event::Listen(overrid, stream))
                }
                _ => {}
            }
        }
